const express = require('express');
const router = express.Router();
const cryptocoinservice = require('../../Services/coinapiprice/coinapiprice');

router.get('/', (req, res) => {
    res.send("Hello from getcoincryptoprice");
});

router.post('/addcryptos', async (req, res, next) => {
    try {
        const listOfUUID = ["Qwsogvtv82FCd", "razxDUgYGNAdQ", "25W7FG7om", "D7B1x_ks7WhV5", "dvUj0CzDZ"];
        const timeinterval = ["24h", "7d", "30d"];
        await cryptocoinservice.fetchAllCryptoData(listOfUUID, timeinterval);

        res.status(200).send({ status: 'success', message: 'Crypto data inserted successfully for all UUIDs and intervals.' });
    } catch (error) {
        res.status(500).send({ error: { message: error.message } });
        next(error);
    }
});

module.exports = router;
